...
object RsInnerAttributeOwnerRegistry {
    private val reg: Map<IElementType, AttrInfo>

    init {
        val regMap = hashMapOf<IElementType, AttrInfo>()

        fun register(element: IElementType, attrs: AttrInfo) {
            regMap[element] = attrs
        }

        register(RsElementTypes.FOREIGN_MOD_ITEM, AttrInfo.Direct)
        register(RsElementTypes.MOD_ITEM, AttrInfo.Direct)
        register(RsFileStub.Type, AttrInfo.Direct)
        register(RsElementTypes.FUNCTION, AttrInfo.Nested(RsElementTypes.BLOCK))
        register(RsElementTypes.IMPL_ITEM, AttrInfo.Nested(RsElementTypes.MEMBERS))
        register(RsElementTypes.TRAIT_ITEM, AttrInfo.Nested(RsElementTypes.MEMBERS))
